...
var dingtalkSDK = void 0;

function run(fn) {
  dingtalkSDK.ready(function () {
    fn(dingtalkSDK.apis);
  });
}

function callTransform$1(names) {
  ...
  return new Promise(function (resolve, reject) {
    run(function (apis) {
      var seq = names.split('.');
      var func = apis;
      seq.forEach(function (next) {
        return func = func[next];
      });
      if (typeof func === 'function') {
        opt.onSuccess = function (data) {
          return resolve(data);
        };
        opt.onFail = function (err) {
          return reject(err);
        };
        func(opt);
      } else {
...
function setCallTransformSource$1(dingtalk) {
  dingtalkSDK = dingtalk || {};
}

var dingtalkCallTransform = {
  callTransform: callTransform$1,
  setCallTransformSource: setCallTransformSource$1
};

var log = logger.log;
var setLog = logger.setLog;
var LogType = logger.LogType;
var callTransform = dingtalkCallTransform.callTransform;
var setCallTransformSource = dingtalkCallTransform.setCallTransformSource;
...
var index = {
  ...
  setCallTransformSource: setCallTransformSource
};
